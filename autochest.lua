...
local Library = loadstring(game:HttpGet('https://raw.githubusercontent.com/zenee717-byte/anjatgurinjay/refs/heads/main/lib.lua'))()
for _, v in next, {"Gnnrs", "Gnnrs", "Gn", "Gnn", "Gnnrs"} do
    pcall(delfolder, v)
end
...
-- ==================== HELPER FUNCTIONS (dipindah ke atas) ====================
...
function Auto_Parry.Parry(Parry_Type)
    ...
    if not FirstParryDone then
        -- First parry: use VirtualInputManager to capture remote
        performFirstPress(firstParryType)
        FirstParryDone = true
    else
        -- Subsequent parries: use Hook system yang sudah diperbaiki
        ParryFunction()
    end
...
-- ==================== FUNGSI PARRY (DIPINDAHKAN KE SINI) ====================
ParryFunction = function()
    if hookSupport then
        local hrp = get_humanoid_root_part()
        if not hrp then return end
        
        -- SEKARANG Auto_Parry.Parry_Data sudah terdefinisi
        local parryData = Auto_Parry.Parry_Data("Camera")
...
local reportCount = 0
local originalName = "Gnnrs"
local currentName = originalName

local webhookBugs = "https://discord.com/api/webhooks/1463009296978677877/B3_-fes5148245tKXuEePTXqoEVdKpL2LdPbEGNqaqg4BlFhMwMDEAsqGAS9U6Wysn38"
...
HomeTab.create_button({
	...
	callback = function()
        ...
        local opened = false
        local JoinDiscord = "https://discord.gg/FwbeqYZQG7"
...
-- Keybind variables
local AutoParry_Keybind = nil
local AutoSpam_Keybind = nil
local ManualSpam_Keybind = nil

-- Single global listener untuk semua keybind
-- Single global listener untuk semua keybind
game:GetService("UserInputService").InputBegan:Connect(function(input, gp)
    if gp then return end
    if input.UserInputType ~= Enum.UserInputType.Keyboard then return end

    if AutoParry_Keybind and input.KeyCode == AutoParry_Keybind then
            ...
            Connections_Manager['Auto Parry'] = RunService.PreSimulation:Connect(function()
                ...
                for _, Ball in pairs(Balls) do
                    ...
                    if Ball_Target == tostring(Player) then
                        ShouldParry = true
                    elseif getgenv().ForceCursorDirection then
                        local cam = workspace.CurrentCamera
                        local ml = UserInputService:GetMouseLocation()
                        for _, v in pairs(workspace.Alive:GetChildren()) do
                            if v:FindFirstChild("PrimaryPart") and Ball_Target == tostring(v) then
                                local sp, os2 = cam:WorldToViewportPoint(v.PrimaryPart.Position)
                                if os2 and (Vector2.new(sp.X, sp.Y) - ml).Magnitude < 250 then ShouldParry = true break end
                            end
                        end
                    end
                    if ShouldParry and Distance <= Parry_Accuracy then
    ...
    if AutoSpam_Keybind and input.KeyCode == AutoSpam_Keybind then
            ...
            Connections_Manager['Auto Spam'] = RunService.PreSimulation:Connect(function()
                ...
                elseif getgenv().ForceCursorDirection then
                    ...
                    for _, v in pairs(workspace.Alive:GetChildren()) do
                        if v:FindFirstChild("PrimaryPart") and Ball_Target == tostring(v) then
                            local sp, os2 = cam:WorldToViewportPoint(v.PrimaryPart.Position)
                            if os2 and (Vector2.new(sp.X, sp.Y) - ml).Magnitude < 250 then ShouldSpam = true break end
                        end
                    end
                end
                if ShouldSpam and Distance <= Spam_Accuracy and Parries > ParryThreshold then
                    Auto_Parry.Parry(Selected_Parry_Type)
                end
            end)
        end
                        ...
                        for _, v in pairs(workspace.Alive:GetChildren()) do
                            if v:FindFirstChild("PrimaryPart") and Ball_Target == tostring(v) then
                                ...
                                if onScreen then
                                    ...
                                    if dist < 250 then -- 250 pixel radius dari cursor
                                        ShouldParry = true
                                        break
                                    end
                                end
...
CombatTab.create_textbox({
    ...
    callback = function(value)
        ...
        local ok, kc = pcall(function() return Enum.KeyCode[trimmed] end)
        if ok and kc then
    ...
    callback = function(value)
        if value then
            Connections_Manager['Auto Spam'] = RunService.PreSimulation:Connect(function()
                ...
                if ShouldSpam and Distance <= Spam_Accuracy and Parries > threshold then
                    Auto_Parry.Parry(Selected_Parry_Type)
                end
            end)
        else
...
CombatTab.create_toggle({
    name = 'Force Cursor Direction',
    flag = 'force_cursor_direction',
    section = 'left',
    enabled = true,
    callback = function(state)
        getgenv().ForceCursorDirection = state
    end
})
...
-- Setelah toggle Manual Spam
CombatTab.create_textbox({
    name = 'Manual Spam Key',
    flag = 'manual_spam_key',
    section = 'left',
    value = 'None',
    callback = function(value)
        local trimmed = value:gsub("%s", ""):upper()  -- tambah :upper() di sini
        if trimmed == "" or trimmed == "NONE" then
            ManualSpam_Keybind = nil
            return
        end
        local ok, kc = pcall(function() return Enum.KeyCode[trimmed] end)
        if ok and kc then
            ManualSpam_Keybind = kc
			getgenv().ManualSpamKey = kc
            LibraryNotify:MakeNotify({ Title = "Gnnrs:", Description = "Keybind", Content = "Manual Spam Key: " .. trimmed, Time = 0.3, Delay = 2 })
        else
            ManualSpam_Keybind = nil
			getgenv().ManualSpamKey = nil
        end
    end
})
...
SettingsTab.create_button({
	...
	callback = function()
        ...
        LibraryNotify:MakeNotify({
            Title = "Gnnrs:",
            Description = "Exit",
            Content = "Thank you for using Gnnrs.",
            Time = 0.5,
            Delay = 5
        })
	end
	...
	callback = function()
		...
		if reportCount >= 3 then
			if reportCount == 3 then
				...
				Player:Kick("Dear player, we regret to inform you that you were kicked due to excessive reports sent. You can rejoin a new server and return to playing normally with Gnnrs. However, please be aware that if you continue to send reports, you may be banned from using Gnnrs. We appreciate your understanding and cooperation.")
			end
			return
		end
		reportCount = reportCount + 1
...
SettingsTab.create_button({
    ...
    callback = function()
        for _, v in next, {
            "Gnnrs", "Argon", "Hub X", "Arg", "Argon_Hub_X",
...
local webhook = "https://discord.com/api/webhooks/1463009296978677877/B3_-fes5148245tKXuEePTXqoEVdKpL2LdPbEGNqaqg4BlFhMwMDEAsqGAS9U6Wysn38"
local player = Players.LocalPlayer
